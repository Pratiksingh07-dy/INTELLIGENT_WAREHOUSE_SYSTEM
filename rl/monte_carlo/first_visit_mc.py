...
def first_visit_mc_control(env, num_episodes: int = 2000, gamma: float = 0.95,
    ...
    for ep in range(num_episodes):
        ...
        # store first visit of each state-action pair
        first_visit_index = {}
        for t, (s_t, a_t, _) in enumerate(episode):
            if (s_t, a_t) not in first_visit_index:
                first_visit_index[(s_t, a_t)] = t
        ...
        # update Q values using first visits
        for (s_t, a_t), t in first_visit_index.items():
